...
if __name__ == '__main__':
    ...
    # -> [좌표 start]는 이미 비용이 안에 내포 되어있다.
    # -> 4개의 start -> 4C2개의 edge 중에 4-1 3개만 뽑히도록 해야한다. 일단 4C2로 만들면서, 비용을 계산하여 edge정보를 만든다.

    # 간선정보의 기록은, graph인접행렬 or 인접빈행렬 or dict에 저장하면 된다.
    # -> 이미 주어진 간선정보는 최소비용트리에 계산안되도록 cost 0으로 미리 추가해놓고 -> 이미 차있으면 건너띄게 해야한다.
    # -> 인접빈행렬을 준비해서 처리하자. -> [ node수 + 1만큼 빈배열을 가진 x]를 만들어주면 된다.
    ...
    for _ in range(e):
        ...
        # 인접빈행렬에 cost까지 포함 기록시, (cost, child)의 튜플로 기록함을 기억하자.
        graph[u].append((0, v))
        graph[v].append((0, u))

    # 최소신장트리를 만드려면, 모든node를 연겨랗는 edge정보가 존재하고 그 사이에서 n-1개만 최소비용순으로 & 싸이클없도록 구성해야한다.
    # -> 좌표로 주어지는 node는 내부에 이미 간선정보를 포함하고 있으며
    # -> [combination을 이용해서, nC2개의 간선정보(parent->child 양방향) 및 cost(parent,v의 좌표배열에서 계산)]까지 만들어낸다.
    ...
    def combination(prev_cnt, prev_position, selected_tuple):
        if prev_cnt == 2:
            ...
            a, b = selected_tuple
            # 인접행렬 기록전에, 이미 존재하는지 검사 -> parent or child 아무거나 1개만 해도 된다.
            # -> 존재한다면, 그 정보(cost 0으로 이미 연결된 간선)을 MST용 간선정보로 모은다.
            for cost, v in graph[a]:
                if v == b:
                    # mst용 간선정보는 [ (cost, parent, child)의 tuple들]로 구성되도록 만든다.
    ...
    # -> start(index)별 root_node(value)를 매핑한 배열이다.
